            ...
            {stats.map((stat) => {
                ...
                return (
                    <div
                        ...
                        className={`relative rounded-lg shadow cursor-pointer transition-all duration-200 hover:shadow-lg border-2 overflow-hidden ${selectedStat === stat.id
                            ? `${getSelectedBackgroundClass(stat.id)} border-transparent`
                            : 'bg-white border-transparent hover:border-gray-200'
                            }`}
                        style={{ height: '144px', padding: '24px' }}
                    >
                        {/* Time Period Selector only for Total Customers - positioned at top right */}
                        {stat.id === 'total' && (
